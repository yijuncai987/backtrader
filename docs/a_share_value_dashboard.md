...
- 大盘拥挤度：`当日成交额排名前 5% 个股成交额合计 / 全市场成交额`，样本为当日有成交的沪深京 A 股（前 5% 数量按样本数向上取整），行情成交额不含大宗交易，口径与乐咕乐股「大盘拥挤度」一致。低于 40% 交易分散，40%~50% 集中度上升，高于 50% 进入预警区域。数据完全由本地 `data/a_share/spot/` 行情快照计算，不访问外部接口；快照文件按运行日归档，重复归档的快照（周末、次日凌晨运行）按成交额指纹去重。
